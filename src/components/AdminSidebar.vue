...
<template>
    ...
    >
        ...
        <div class="flex flex-col justify-between items-center p-4 bg-blue-600 dark:bg-blue-800">
            <div class="flex flex-row items-center">
                <img :src="logoImg" alt="GLI Logo" class="w-12 object-contain bg-white rounded" />
                <p v-if="!isMinimized" class="ml-2 font-medium text-white">GLI Platform</p>
            </div>
            <h2 v-if="!isMinimized" class="text-xl font-bold my-2 text-white">관리자 대시보드</h2>
            <span
        ...
        <nav class="menu flex-1 overflow-y-auto">
            <div v-for="item in menuItems" :key="item.path" class="font-bold">
                ...
                >
                    <span class="icon" :class="{ 'mr-3': !isMinimized }">{{ item.icon }}</span>
                ...
                <!-- 하위 메뉴 -->
                <div
                    v-if="!isMinimized && item.children && expandedMenus.includes(item.path)"
...
const userInfo = computed<AdminUser | null>(() => authStore.user);
const isMinimized = ref(false);

const isDark = useDark({
    storageKey: 'admin-theme',
    initialValue: false, // 기본값을 라이트 테마로 설정
});
