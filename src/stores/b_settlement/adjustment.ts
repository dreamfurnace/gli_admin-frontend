...
export interface IPrecheckItem {
    target: string;
    label: string;
    totalCount: number;
    normalCount: number;
...
export interface IUploadRow {
    ...
    file: File | null;
    fileName: string;
    distributor_settle_form_id: number;
    status: 'Parsed' | 'Parsed-S2' | 'Parsing' | 'Error' | 'None' | 'Error-Parsing'; // ✅ 상태 추가
}

type PresignedResponse = {
    upload_urls: Record<'domestic' | 'overseas', string>;
    s3_keys: Record<'domestic' | 'overseas', string>;
};
export const useAdjustmentStore = defineStore('adjustmentStore', () => {
    const selectedMonthId = ref<number | null>(null);
    const precheckList = ref<IPrecheckItem[]>([]);
    const uploadRows = ref<IUploadRow[]>([
        {
            label: '국내정산',
            file: null,
            fileName: '',
    ...
    const initialFileNames = ref<string[]>(['', '']); // 초기 파일 상태 저장
    ...
    async function uploadAllFilesToS3(presigned: PresignedResponse) {
    ...
    async function analyzeFiles(s3_keys: PresignedResponse['s3_keys']) {
        const payload = {
            month_settle_id: selectedMonthId.value,
            s3_keys,
        };
    ...
    async function saveAdjustmentData_Date_or_Files() {
        ...
        uploadRows.value.forEach((row, idx) => {
            if (row.file) {
                ...
                formData.append(
                    idx === 0 ? 'domestic_file_delete' : 'overseas_file_delete',
                    'true'
                );
            }
        });
        ...
        try {
            ...
            res.data.files.forEach(
                (file: { file_id: number; status: string; distributor_settle_form_id: string }) => {
                    // const idx = file.file_id === 1 ? 0 : 1; // distributor_settle_form_id로 구분
                    const idx = file.distributor_settle_form_id === '1' ? 0 : 1;
                    uploadRows.value[idx].status = file.status as
                        | 'Parsed'
                        | 'Parsed-S2'
                        | 'Parsing'
                        | 'Error'
    ...
    function setSelectedMonth(monthId: number) {
    ...
    async function fetchVerificationData() {
        ...
        try {
            ...
            precheckList.value = res.data.verification.map(
                (item: {
                    target: string;
                    check: string;
                    total: number;
            ...
            uploadRows.value.forEach((row) => {
                const fileData = res.data.files.find(
                    (f: { distributor_settle_form_id: number }) =>
                        f.distributor_settle_form_id === row.distributor_settle_form_id
                );
                row.fileName = fileData?.file_name || '';
                row.status = (fileData?.status as 'Parsed' | 'Parsing' | 'None') || 'None';
            });
    ...
    async function fetchDetailTableData_Status(monthSettleId: number) {
        try {
    ...
    async function deleteDetaileTableData(monthSettleId: number, table: string) {
        try {
            ...
            if (table === 'user_settles') {
                ...
                // ✅ 상태와 파일 이름 초기화
                const resetIds: number[] = res.data.reset_form_ids || [];
    ...
    async function processTable(monthSettleId: number, tableKey: string) {
        try {
            ...
            if (res.data.updated_settle_files) {
                res.data.updated_settle_files.forEach((formId: number | string) => {
