...
import type {
  RWAAsset,
  CreateRWAAssetRequest,
  UpdateRWAAssetRequest,
...
class RWAService {
  private basePath = '/api/admin/rwa';

  /**
   * RWA 자산 목록 조회
   */
  async getAssets(filter: RWAAssetFilter = {}): Promise<RWAAssetListResponse> {
    try {
      // 개발 환경에서 모크 데이터 사용
   ...
   * RWA 자산 생성
   */
  async createAsset(data: CreateRWAAssetRequest): Promise<RWAAsset> {
    try {
      if (import.meta.env.VITE_USE_MOCK_API === 'true') {
   ...
   * RWA 자산 수정
   */
  async updateAsset(id: string, data: UpdateRWAAssetRequest): Promise<RWAAsset> {
    try {
      if (import.meta.env.VITE_USE_MOCK_API === 'true') {
   ...
   * RWA 자산 활성화/비활성화
   */
  async toggleAssetStatus(id: string, isActive: boolean): Promise<RWAAsset> {
    try {
      if (import.meta.env.VITE_USE_MOCK_API === 'true') {
   ...
   * RWA 투자 통계 조회
   */
  async getInvestmentStats(): Promise<RWAInvestmentStats> {
    try {
      if (import.meta.env.VITE_USE_MOCK_API === 'true') {
   ...
   * RWA 자산 투자자 목록 조회
   */
  async getAssetInvestors(assetId: string): Promise<RWAAssetInvestor[]> {
    try {
      if (import.meta.env.VITE_USE_MOCK_API === 'true') {
   ...
   * RWA 자산 성과 조회
   */
  async getAssetPerformance(assetId: string): Promise<RWAAssetPerformance> {
    try {
      if (import.meta.env.VITE_USE_MOCK_API === 'true') {
  ...
  private getMockAssets(filter: RWAAssetFilter): RWAAssetListResponse {
    let filteredAssets = [...this.mockAssets];

    // 필터링
    if (filter.search) {
      filteredAssets = filteredAssets.filter(asset => 
        asset.name.includes(filter.search!) || 
    ...
    if (filter.sortBy) {
      filteredAssets.sort((a, b) => {
        const aVal = a[filter.sortBy as keyof RWAAsset];
        const bVal = b[filter.sortBy as keyof RWAAsset];
  ...
  private getMockAsset(id: string): RWAAsset {
  ...
  private createMockAsset(data: CreateRWAAssetRequest): RWAAsset {
  ...
  private updateMockAsset(id: string, data: UpdateRWAAssetRequest): RWAAsset {
  ...
  private deleteMockAsset(id: string): void {
    const index = this.mockAssets.findIndex(a => a.id === id);
    if (index === -1) {
      throw new Error('Asset not found');
    }
  ...
  private getMockInvestmentStats(): RWAInvestmentStats {
    return {
  ...
  private getMockAssetInvestors(assetId: string): RWAAssetInvestor[] {
    return [
      {
        id: '1',
        userId: 'user1',
  ...
  private getMockAssetPerformance(assetId: string): RWAAssetPerformance {
    return {
      assetId,
      currentReturn: 7.2,
      projectedReturn: 8.5,
